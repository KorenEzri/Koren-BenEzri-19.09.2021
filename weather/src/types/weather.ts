interface IMetricImperial {
  Value: number;
  Unit: string;
  UnitType: number;
}

interface ITempratureType {
  Metric: IMetricImperial;
  Imperial: IMetricImperial;
}
interface ITempratureMinMax {
  Minimum: IMetricImperial;
  Maximum: IMetricImperial;
}
...
export interface IFullConditions {
  LocalObservationDateTime: string;
  EpochTime: number;
  WeatherText: string;
  WeatherIcon: number;
